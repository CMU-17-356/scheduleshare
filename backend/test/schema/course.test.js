...
describe('Courses should not exist without a courseID and name', function() {
  ...
  it('Should not create a course with no courseID', (done) => {
    new Course({name: "Software Engineering for Startups"}).validate(function(err) {
      expect(err).to.exist;
    });
    done();
  });
...
describe('Courses with courseID and name should be able to be created', function() {
  it('Should create a course with required params', (done) => {
    const course = new Course({courseID: 17356, name: "Software Engineering for Startups"});
