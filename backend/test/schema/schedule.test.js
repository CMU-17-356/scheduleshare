...
describe('Schedules should not exist with bogus arrays', function() {
  ...
  it('Should not create a schedule with non-course strings in array', (done) => {
    const schedule = new Schedule({user_id:user1._id, courses:["abc", "def"]});
  ...
  it('Should not create a schedule with non-course strings in array and a course string', (done) => {
    const schedule = new Schedule({user_id:user1._id, courses:["15-213", "def"]});
    schedule.validate(function(err){
      expect(err).to.exist;
    });
    done(); 
...
describe('Schedules can exist with valid parameters', function() {
  ...
  before(function(done) {
    ...
    it('Should create a schedule with required params (empty array)', (done) => {
        const schedule = new Schedule({user1:user1._id, courses:[]});
        assert(schedule.validate());
        done();
    });

    it('Should create a schedule with required params (non-empty array)', (done) => {
