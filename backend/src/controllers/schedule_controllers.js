...
const view_user_by_id = (req, res) => {
    const users = User.find({_id: req.params._id}, (err, result) => {
        if (err) {
            res.status(400).send(err);
        } else {
            res.status(200).send(result);
...
const add_user = (req, res) => {
    const user = new User(req.body).save((err, result) => {
        if (err) {
            res.status(400).send(err);
        } else {
...
const update_user_by_id = (req, res) => {
    const user = User.updateOne({_id: req.params._id}, req.body, (err, result) => {
        if (err) {
            res.status(400).send(err);
        } else {
