...
const get_first_x_courses = (req, res) => {
    ...
    while (result_list.length < parseInt(req.params._id)) {
        let new_course_id = initial_course_first_half.toString() + "-" + initial_course_second_half.toString();
        ...
        if (initial_course_first_half > 99 || result_list.length > parseInt(req.params._id)) {
            break;
        } 
    }
